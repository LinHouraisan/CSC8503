...
std::tuple<std::vector<CollisionDetection::CollisionInfo>, std::vector<CollisionDetection::CollisionInfo>>
PhysicsSystem::Update(float dt) {
    ...
    auto ret = UpdateCollisionList(); // Remove any old collisions
...
*/
std::tuple<std::vector<CollisionDetection::CollisionInfo>, std::vector<CollisionDetection::CollisionInfo>>
PhysicsSystem::UpdateCollisionList() {
    std::vector<CollisionDetection::CollisionInfo> beginCollisions;
    std::vector<CollisionDetection::CollisionInfo> endCollisions;
    for (std::set<CollisionDetection::CollisionInfo>::iterator i = allCollisions.begin(); i != allCollisions.end();) {
        if ((*i).framesLeft == numCollisionFrames) {
            i->a->OnCollisionBegin(i->b);
            i->b->OnCollisionBegin(i->a);
            beginCollisions.push_back(*i);
        }
        ...
        if ((*i).framesLeft < 0) {
            ...
            endCollisions.push_back(*i);
            i = allCollisions.erase(i);
        } else {
            ++i;
        }
    }
    return {beginCollisions, endCollisions};
}
...
void PhysicsSystem::BasicCollisionDetection() {
    std::vector<GameObject *>::const_iterator moveFirst;
    std::vector<GameObject *>::const_iterator moveLast;
    gameWorld.GetMoveObjectIterators(moveFirst, moveLast);
    for (auto i = moveFirst; i != moveLast; ++i) {
        assert((*i)->GetPhysicsObject());
        for (auto j = i + 1; j != moveLast; ++j) {
            assert((*j)->GetPhysicsObject());
            CollisionDetection ::CollisionInfo info;
            if (CollisionDetection ::ObjectIntersection(*i, *j, info)) {
                if (GetGlobalConfig().printCollisionInfo)
                    std ::cout << " Collision between " << (*i)->GetName() << " and " << (*j)->GetName() << std ::endl;

                switch (GetGlobalConfig().collisionResponseMethod) {
    ...
    std::vector<GameObject *>::const_iterator fixedFirst;
    std::vector<GameObject *>::const_iterator FixedLast;
    gameWorld.GetFixedObjectIterators(fixedFirst, FixedLast);
    for (auto i = moveFirst; i != moveLast; ++i) {
        assert((*i)->GetPhysicsObject());
        for (auto j = fixedFirst; j != FixedLast; ++j) {
            assert((*j)->GetPhysicsObject());
            CollisionDetection ::CollisionInfo info;
            if (CollisionDetection ::ObjectIntersection(*i, *j, info)) {
                if (GetGlobalConfig().printCollisionInfo)
...
void NCL::CSC8503::PhysicsSystem::ProjectionResolveCollision(GameObject &a, GameObject &b,
    ...
    // seperate those by just moving back
    Vector3 moveA = -p.normal * p.penetration;
    Vector3 moveB = p.normal * p.penetration;

    // if we just end of here and apply the movement to object A and B,
    // something will be weird such as that the floor is pushed by collision object, etc.

    // We can improve this separation concept by, instead of just separating the objects out by an equal
    // amount, instead separating them out by an amount proportional to their mass

    // equation 1: contributionMassA + contributionMassB = 1.0
    // equation 2: massA / contributionMassA = massB / contributionMassB
    //          => inverseMassA * contributionMassA = inverseMassB * contributionMassB
    // => contributionA = inverseMassB / (inverseMassA + inverseMassB)
    float contributionA = physA->GetInverseMass() / (physA->GetInverseMass() + physB->GetInverseMass());
    float contributionB = 1.0 - contributionA;

    moveA *= contributionA;
    moveB *= contributionB;

    a.GetTransform().SetPosition(transformA.GetPosition() + moveA);
    b.GetTransform().SetPosition(transformB.GetPosition() + moveB);
}

void NCL::CSC8503::PhysicsSystem::PenaltyResolveCollision(GameObject &a, GameObject &b,
                                                          CollisionDetection::ContactPoint &p) const {
    PhysicsObject *physA = a.GetPhysicsObject();
    PhysicsObject *physB = b.GetPhysicsObject();
    ...
    // we assume the spring constant k is 1.0
    float k = 1.0f;

    // ForceA has an opposite direction of the normal
    Vector3 forceA = -p.normal * k * physA->GetInverseMass();

    // ForceB has a same direction of the normal
    Vector3 forceB = p.normal * k * physB->GetInverseMass();

    physA->AddForceAtPosition(forceA, p.localA + transformA.GetPosition());
    physB->AddForceAtPosition(forceB, p.localB + transformB.GetPosition());
}
