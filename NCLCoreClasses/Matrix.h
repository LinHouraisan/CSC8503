...
struct MatrixTemplate {
    ...
    template <uint32_t rhsRows, uint32_t rhsCols>
    MatrixTemplate(const MatrixTemplate<T, rhsRows, rhsCols> &rhs) {
        static_assert(rhsRows <= rows && rhsCols <= cols);
...
VectorTemplate<T, 3> operator*(const MatrixTemplate<T, 3, 3> &mat, const VectorTemplate<T, 3> &v) {
    return VectorTemplate<T, 3>(v.x * mat.array[0][0] + v.y * mat.array[1][0] + v.z * mat.array[2][0],
...
constexpr MatrixTemplate<T, 4, 4> Inverse(const MatrixTemplate<T, 4, 4> &mat) {
    ...
    det = (-det3_201_123 * mat.array[3][0] + det3_201_023 * mat.array[3][1] - det3_201_013 * mat.array[3][2] +
           det3_201_012 * mat.array[3][3]);
...
template <typename T>
constexpr MatrixTemplate<T, 4, 4> View(const VectorTemplate<T, 3> &cameraPos, const VectorTemplate<T, 3> &lookAtPos,
