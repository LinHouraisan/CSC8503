...
*/
void Camera::UpdateAngleByAxis(float dt) {
    if (!activeController) {
        return;
    }
...
void NCL::Camera::UpdatePositionByKey(float dt) {
...
Matrix4 Camera::BuildViewMatrix() const {
    ...
    return Matrix::Rotation(-pitch, Vector3(1, 0, 0)) * Matrix::Rotation(-yaw, Vector3(0, 1, 0)) *
